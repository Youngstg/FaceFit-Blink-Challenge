...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{Figure/diagramproses.png}
    \caption{Alur Eksekusi Program}
    \label{fig:Alur Eksekusi}
\end{figure}
...
Integrasi modul dalam game loop menunjukkan bahwa sistem bergantung pada sinkronisasi antara deteksi wajah dan logika permainan. \textit{face\_processing} menghasilkan landmark dan nilai EAR pada setiap frame, kemudian \textit{game\_controller} menentukan status kedipan dan memicu aksi seperti menghentikan objek jatuh atau memperbarui posisi anchor. Ketepatan pembacaan landmark berpengaruh langsung pada stabilitas pergerakan objek. Ketika deteksi wajah tidak konsisten, posisi \textit{falling\_face\_part} dapat bergetar saat mengikuti pergerakan kepala. Kondisi ini menunjukkan bahwa integrasi modul masih sensitif terhadap noise kamera dan perubahan posisi wajah.


Dari sisi kinerja, pembaruan objek dan rendering dilakukan pada loop yang sama sehingga setiap modul saling mempengaruhi FPS akhir. Proses yang paling berat berada pada deteksi landmark, sementara modul permainan hanya menambah sedikit beban. Pengujian menunjukkan bahwa selama integrasi berjalan mulus, respons kedipan cepat dan jatuhnya objek tetap sinkron. Namun ketika pencahayaan buruk atau landmark tidak stabil, permainan mengalami jeda kecil sebelum objek menempel ke wajah. Ini menandakan bahwa performa sistem sangat ditentukan oleh kualitas input visual dan efisiensi komunikasi antar modul.

\subsection{Analisis Implementasi}

Hasil implementasi menunjukkan bahwa sistem mampu memproses input real-time dengan stabil tanpa mengalami lag yang signifikan. Deteksi kedipan bekerja dengan cepat dan tingkat akurasi yang baik berkat penggunaan kalibrasi EAR dinamis yang meningkatkan adaptabilitas sistem terhadap variasi pengguna dan kondisi pencahayaan. Integrasi antara deteksi wajah dan logika permainan berjalan sinkron sehingga proses rendering tidak mengalami penurunan FPS yang berarti.\par

Dari perspektif interaksi, pengguna dapat berinteraksi secara natural tanpa memerlukan perangkat tambahan selain kamera. Tracking objek yang sudah terpasang mengikuti pergerakan wajah dengan presisi tinggi berkat perhitungan transformasi berbasis anchor. Masking warna kulit yang dihasilkan dari sampling area pipi dan dahi memberikan tampilan yang natural pada wajah yang di-mask.\par

Dari sisi arsitektur perangkat lunak, desain modular yang diterapkan memudahkan pengembangan lanjutan dan pemeliharaan kode. Implementasi caching pada asset loader meningkatkan performa loading sehingga tidak terjadi delay saat menampilkan elemen-elemen UI yang sama berulang kali. Kekurangan yang ditemukan antara lain yaitu anchor ketika part face dikedipkan dan berhenti tidak pada wajah bisa diatas dan dibawah wajah maka anchor tidak terlalu bekerja dan part face tidak mengikuti wajah dengan baik.\par
